...
public class AnswerServiceImpl extends BaseServiceImpl<Answer> implements AnswerService {
	...
	// 在这里来计算成绩
	/**
	 * 总分是100分 单选多少分，多选该有多少分
	 * 
	 * ---? 这里应该还要知道taoti的ID才行。然后获得对应套题的题目数 再由【总分100/题目数】得到每道题的分数
	 */
	@Resource
	private ScoreService scoreservice;

	@Resource
	private LessonService lessonservice;// 用来找到lessonID对应的课程，并给到score
	...
	public void saveAnswers(List<Answer> handleAnswer, Score score) {
		if (!handleAnswer.isEmpty() && handleAnswer.size() > 0) {
			...
			score.setLessonName(lessonservice.getEntity(score.getLessonID()).getLessonName());
			// 计算单选成绩和多选成绩
			float singleResult = caculateScore(score.getStuNum(), 40f, score.getPaperID(), "单选题", score.getTest_Date());
			float multipleResult = caculateScore(score.getStuNum(), 60f, score.getPaperID(), "多选题",
					score.getTest_Date());
	...
	public float caculateScore(String stuNum,float total,int TaotiId,String type,Date TestDate){
		...
		String hql="select count(*) from Question where taoti.id=? and type=?";
		//找到了Taoti中对应的题目总数
		...
		//获取该考生提交答案中的正确题目总数
		String hql1="select count(*) from Answer where stuNum=? and type=? and taotiId=? and examdate=? and isTrue=?";
		long right=(Long) this.UniqueResult(hql1, stuNum,type,TaotiId,TestDate,true);
		//求得其获得的总分
